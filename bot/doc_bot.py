...
def _split(records, skip):
    """(kept, excluded), excluded as (name, reason) pairs."""
    kept, excluded = [], []
    for r in records:
...
def run(scenario, krkn_hub_root, website_root, krkn_root: str | Path = "krkn",
    ...
    if (scn / "env.sh").exists():
        recs, excluded = _split(extract_env_params(scn / "env.sh"), skip)
        gaps += [(scenario, "krkn-hub", name, EXCLUDED, reason, "")
    ...
    if (scn / "krknctl-input.json").exists():
        ...
        gaps += [(scenario, "krknctl", name, EXCLUDED, reason, "")
                 for name, reason in excluded]
        if recs:
